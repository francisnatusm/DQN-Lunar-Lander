...
def test_network(target):
    ...
    def _shape_as_list(obj):
        # Accept TensorShape, objects with .shape, or plain tuples
        s = None
    ...
    assert _shape_as_list(target.input) == [None, state_size], \
        f"Wrong input shape. Expected [None,  {state_size}] but got {_shape_as_list(target.input)}" 
    ...
    for layer in target.layers:
        ...
        assert out_shape_list == expected[i][1], \
            f"Wrong number of units in layer {i}. Expected {expected[i][1]} but got {out_shape_list}"
